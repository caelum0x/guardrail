...
| `guardrail-api`      | axum read API over the event store + run report (30 routes) | No |
...
All 30 routes are read-only `GET`s over the event store and run report. Source
of truth: `apps/guardrail-api/src/server.rs::build_app`.
...
`guardrail-api` responses. 29 pages (`dashboard/src/app/*`, routed via
