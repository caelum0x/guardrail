...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Scorecard { config } => run_scorecard(&config)?,
        Commands::SdkCatalog => run_sdk_catalog()?,
        Commands::AgentServices { config } => run_agent_services(&config)?,
        Commands::AgentCard { config } => run_agent_card(&config)?,
        Commands::JobSimulator { config } => run_job_simulator(&config)?,
...
fn run_scorecard(config_path: &str) -> anyhow::Result<()> {
    let config = read_json_report(config_path)
...
fn run_sdk_catalog() -> anyhow::Result<()> {
...
fn run_agent_services(config_path: &str) -> anyhow::Result<()> {
    let config = read_json_report(config_path)
...
fn run_agent_card(config_path: &str) -> anyhow::Result<()> {
    let config = read_json_report(config_path)
...
fn run_job_simulator(config_path: &str) -> anyhow::Result<()> {
    let config = read_json_report(config_path)
