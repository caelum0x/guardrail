...
| **momentum-volatility-blend-bsc** | Same regime classifier + risk envelope, but a **momentum × volatility-quality blend**: favour names that are *both* strongly trending (aligned rising EMA stack + positive MACD histogram + risk-adjusted return) *and* sitting in a **healthy realised-volatility band**. A bell-shaped `vol_quality` gate (1.0 in ~50-80% annualised, ramping to 0 below the 40% floor and above the 110% ceiling) **multiplies** the momentum leg — `blend_tilt = momentum_leg * vol_quality` — so **dead-vol** names (no trend energy) and **blow-off-vol** names (parabolic exhaustion) are demoted even with strong momentum, and a trendless name scores ~0. ATR(14) stops; exposure peaks in breakout (1.1) and risk_on, cut in chop/risk_off. | risk_on, risk_off, chop, breakout | 4 | [`momentum-volatility-blend/`](./momentum-volatility-blend) |
...
for d in ['../skills/cmc-regime-routed-alpha/examples',
          ...
          '../skills/momentum-volatility-blend/examples']:
