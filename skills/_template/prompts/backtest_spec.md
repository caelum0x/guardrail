# Backtest Specification Prompt — <PLACEHOLDER_SKILL_NAME>  (TEMPLATE)

Describe — and produce — a defensible backtest of this strategy so a judge can
reproduce and trust the reported numbers.

## Replay data

- **Universe**: the 20 eligible BSC tokens (`configs/eligible_assets.bsc.json`).
- **Signal**: <PLACEHOLDER how this skill's raw signal is replayed alongside the
  market snapshot, and its source (Rust file or CMC endpoint)>.
...
- **Granularity**: hourly candles; regime + signal tilt + rebalance evaluated each
  cycle.
- **Period**: a minimum 90-day window covering at least one risk_on, one risk_off,
  and one chop stretch; report start/end timestamps.
...
- <PLACEHOLDER any signal-specific entry/exit gate>.
- Drawdown throttle at 22% total drawdown (block new buys), kill switch latches at
  24% (halt trading).
...
- **Signal attribution**: PnL attributable to this skill's tilt vs the base alpha
  (compare against a base-alpha-only run on the same window).
...
per-regime attribution table, a signal-attribution breakdown, and an explicit
list of every risk-gate activation.
